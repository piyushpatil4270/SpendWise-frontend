...
const ExpenseTable = ({isPremium,setPremium}) => {
  ...
  const getStatsMonthly = async () => {
    try {
      ...
      const res = await axios.post("https://www.expensetracker2.kesug.com/expense/getbyMonthGrouped",{date:new Date()}, {
  ...
  const getStatsYearly = async () => {
    try {
      const userToken = localStorage.getItem("token");
      const res = await axios.post("https://www.expensetracker2.kesug.com/expense/getbyYear",{date:new Date()}, {
        headers: { Authorization: userToken },
      });
